...
- ✅ **Phase 2 — Content pipeline & TTS:** COMPLETE 2026-06-18. Operator-authored 4,663-word CSV loaded into words. Browser TTS (PR #22) + pre-generated Google TTS Neural2-F audio (all 4,663 words, bucket: word-audio) both shipped. Vocabulary browse paginated (PR #24). Next: Phase 3 game layer.
...
UX fixes and audio complete. Shipped PR #26 (dashboard CTAs split into "Learn New Words" / "Review Words" with matching button styles), PR #27 (learn queue filter fix — PostgREST embedded left-join null filter doesn't work; replaced with two-step fetch + .not('id', 'in', (...)) exclusion), PR #28 (state file update banking the left-join gotcha as a named precedent). Stage 2 audio complete: all 4,663 words generated via Google Cloud TTS Neural2-F (female, American English), uploaded to Supabase Storage bucket `word-audio`, audio_url fully populated on every words row. Script: scripts/generate-tts-audio.ts, run via `pnpm generate-tts`, resumable. GOOGLE_TTS_API_KEY in .env.local (not committed). Browser TTS remains as fallback only — no code change needed when real audio is present. Also updated operator's personal reference docx (Idea to App The Director System.docx) with current Director prompt and Codex CLI local workflow description — not part of the repo. Phase 2 is now fully complete: content, TTS Stage 1 + Stage 2, pagination all shipped.
